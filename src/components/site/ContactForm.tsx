  ...
  return (
    <form className="contact-form" onSubmit={submit} noValidate>
      <div className="form-grid">
        <label>
          <span>姓名 *</span>
          <input name="name" autoComplete="name" placeholder="如何称呼你" />
        </label>
        <label>
          <span>公司</span>
      ...
      <label>
        <span>联系方式 *</span>
        <input name="contact" autoComplete="email" placeholder="邮箱、手机号或微信号" />
      </label>
      <label>
        <span>项目需求 *</span>
        <textarea name="need" rows={6} placeholder="请简单描述业务目标、当前阶段和希望解决的问题" />
      </label>
      {state === 'error' && <p className="form-error">请填写姓名、联系方式和项目需求后再提交。</p>}
      {state === 'ready' && (
        <p className="form-success">邮件客户端已打开。如果没有自动打开，请直接发送邮件到下方邮箱。</p>
      )}
